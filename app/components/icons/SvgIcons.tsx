...
// from the exact Ionicons.ttf @expo/vector-icons vendors, so the rendered
// shape is pixel-identical to what the font glyph produced.
const makeSvgIconComponent = (library: string) => {
  const Component = ({ name, size = 20, color = "#000", style }: SvgVectorIconProps) => {
    ...
    // always square — treat `size` as the height and derive width from the
    // viewBox's own aspect ratio so non-square glyphs aren't stretched.
